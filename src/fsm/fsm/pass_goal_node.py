...
class PassGoalNode(LifecycleNode):
    def __init__(self):
        # 노드 이름을 'pass_goal_node'로 변경하여 FSM 테이블에 맞춥니다.
        super().__init__("pass_goal_node")

        # 인스턴스 변수들을 초기화합니다.
        self.ball_topic = None
        self.human_topic = None
        ...
        self.human_msg: Optional[PointStamped] = None
        self.ball_sub = None
        self.human_sub = None
        self.goal_pub = None
        self.timer = None
    ...
    def on_configure(self, state: LifecycleState):
        ...
        """
        self.get_logger().info("PassGoalNode: 구성(on_configure) 상태로 전환 중...")

        self.declare_parameter("ball_topic", "/ball")
        self.declare_parameter("human_topic", "/human")
        ...
        self.human_sub = self.create_subscription(PointStamped, self.human_topic, self._human_cb, qos)
        self.goal_pub = self.create_publisher(Pose, self.goal_topic, 10)
        
        self.get_logger().info("PassGoalNode: 리소스 초기화 완료.")
    ...
    def on_activate(self, state: LifecycleState):
        ...
        """
        self.get_logger().info("PassGoalNode: 활성화(on_activate) 상태입니다. 패스 목표 계산을 시작합니다.")
    ...
    def on_deactivate(self, state: LifecycleState):
        ...
        """
        self.get_logger().info("PassGoalNode: 비활성화(on_deactivate) 상태입니다. 패스 목표 계산을 중지합니다.")
    ...
    def on_cleanup(self, state: LifecycleState):
        ...
        """
        self.get_logger().info("PassGoalNode: 정리(on_cleanup) 상태로 전환 중...")
        self.destroy_subscription(self.ball_sub)
        self.destroy_subscription(self.human_sub)
    ...
    def on_shutdown(self, state: LifecycleState):
        self.get_logger().info("PassGoalNode: 종료(on_shutdown) 중...")
        return TransitionCallbackReturn.SUCCESS
    ...
    def _human_cb(self, msg: PointStamped):
        self.human_msg = msg
    ...
    def _step(self):
        ...
        if self.ball_msg is None or self.human_msg is None:
            return
        if not (self._age_ok(self.ball_msg) and self._age_ok(self.human_msg)):
            return

        ball_b = self._to_target_frame(self.ball_msg)
        human_b = self._to_target_frame(self.human_msg)
        if ball_b is None or human_b is None:
            return

        bx, bz = self._extract_xz(ball_b)
        px, pz = self._extract_xz(human_b)
...
def main(args=None):
    ...
    pass_goal_node = PassGoalNode()
    rclpy.spin(pass_goal_node)
    rclpy.shutdown()
